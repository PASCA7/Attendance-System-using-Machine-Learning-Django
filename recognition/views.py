...
def total_number_student():
	qs=User.objects.all()
	return (len(qs) -1)
	# -1 to account for maneesh



def student_present_today():
	today=datetime.date.today()
...
def dashboard(request):
	if(request.user.username=='maneesh'):
		print("admin")
...
def view_attendance_home(request):
	total_num_of_emp=total_number_student()
	emp_present_today=student_present_today()
